		...
		try
		{
			using (record)
			{
				...
				try
				{
					...
					// v2.2: Also stamp the bookmark onto the DTO so EventProcessorWorker can
					// UPSERT it in the SAME SqliteTransaction as the event insert (unified
					// commit boundary — see EventProcessorWorker.PersistBatchAsync). The
					// legacy PeriodicTimer path via TrackPendingBookmark/FlushPendingBookmarksAsync
					// remains active as a fallback for DTOs that get dropped by the ring buffer
					// overflow guard (those never reach the processor, so unified commit cannot
					// advance their bookmark). Both paths converge on the same UPSERT statement
					// in BookmarkStore; last-write-wins is safe because Windows bookmarks are
					// monotonically increasing per channel.
					dto.BookmarkXml = bookmarkXml;
				}
				catch (Exception ex)
				{
