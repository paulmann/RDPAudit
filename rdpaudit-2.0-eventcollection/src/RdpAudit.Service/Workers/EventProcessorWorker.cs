...
//          SQLite inside a single explicit transaction. As of v2.3, the last bookmark per
//          channel in the batch is UPSERTed through BookmarkStore.SaveBatchInSameTransactionAsync
//          inside the same SqliteTransaction as the event inserts, so the event batch and its
//          bookmark cross ONE durability boundary. The legacy EventCollectorWorker
//          PeriodicTimer flush loop is intentionally left in place as a fallback for events
//          that were dropped upstream by the ring buffer overflow guard (they never reach the
//          processor, so unified commit cannot advance their bookmark).
...
//          v2.3.0: Unified commit boundary for bookmark + event batch. PersistBatchAsync now
//          gathers the LAST bookmark per channel out of the batch, optimistically updates the
//          BookmarkStore cache before BeginTransactionAsync, UPSERTs every collected bookmark
//          through the caller-owned SqliteTransaction after the two fact-upserter
//          SaveChangesAsync barriers, and either MarkCommitted-s (on tx.CommitAsync success) or
//          RollbackCaches every entry (on transaction failure). BookmarkStore is optional in
//          the constructor so unit tests that isolate DrainBatchAsync keep passing with the
//          existing null-arg call sites (unchanged behaviour when bookmarkStore is null).
...
//          BookmarkStore (optional — enables unified commit path)
...
//          "normalize -> address upsert -> connection facts -> auth facts -> bookmarks ->
//          commit" ordering. Peer artefacts that also want the unified boundary (e.g. shard
//          watermark) plug in immediately before tx.CommitAsync following the bookmark pattern:
//          cache-update BEFORE BeginTransaction, persist inside tx, rollback-cache on catch.
...
public sealed class EventProcessorWorker : BackgroundService
{
	...
	/// <summary>
	/// Optional bookmark store. When present, PersistBatchAsync UPSERTs the last bookmark per
	/// channel through the SAME SqliteTransaction as the event inserts, so the batch and its
	/// bookmark cross a single durability boundary. When null (unit tests that isolate
	/// DrainBatchAsync), the persist path is exactly identical to v2.2 — no bookmark work.
	/// </summary>
	...
	private async Task PersistBatchAsync(List<RawEventDto> dtos, CancellationToken ct)
	{
		...
		// ── Unified-commit bookmark collection ─────────────────────────────────────
		// Collect the LAST bookmark per channel out of the entire DTO batch (walking in
		// forward order so later DTOs overwrite earlier ones — Windows bookmarks are
		// monotonically increasing per channel). We MUST use the DTO batch, not the
		// normalized `entities` list: even DTOs whose normalization failed had already
		// been read from the log, and their bookmark must still advance so the collector
		// does not replay them on restart. Result is a small heap allocation proportional
		// to the number of DISTINCT channels in this batch (typically 1..3) — not the
		// number of events. On empty batches (dtos.Count == 0), nothing is collected and
		// the entire unified-commit branch is skipped.
		Dictionary<string, string>? pendingBookmarks = null;
		Dictionary<string, string?>? previousBookmarks = null;
		bool unifiedBookmarkPath = _bookmarkStore is not null && dtos.Count > 0;

		if (unifiedBookmarkPath)
		{
			foreach (RawEventDto dto in dtos)
			{
				string? bookmark = dto.BookmarkXml;
				if (bookmark is null || bookmark.Length == 0)
				{
					continue;
				}
				...
				pendingBookmarks ??= new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
				pendingBookmarks[channel] = bookmark;
			}

			if (pendingBookmarks is not null && pendingBookmarks.Count > 0)
			{
				// Optimistically advance the in-memory cache BEFORE opening the transaction.
				// UpdateCache returns the previous value; we stash it so RollbackCache can
				// restore it verbatim if the transaction fails.
				previousBookmarks = new Dictionary<string, string?>(pendingBookmarks.Count, StringComparer.OrdinalIgnoreCase);
				foreach (KeyValuePair<string, string> entry in pendingBookmarks)
				{
					string? prev = _bookmarkStore!.UpdateCache(entry.Key, entry.Value);
					previousBookmarks[entry.Key] = prev;
				}
			}
			else
			{
				// No usable bookmarks in this batch (e.g. all DTOs are synthetic backfill).
				// Downgrade to the legacy path so the transaction cost is unchanged.
				unifiedBookmarkPath = false;
			}
		}
		...
		try
		{
			...
			// ── Unified-commit bookmark UPSERT ─────────────────────────────────────────
			// Bookmark write happens AFTER the fact upserters (so a bookmark failure
			// still fails the batch and rolls back the events) but BEFORE tx.CommitAsync
			// (so the SqliteTransaction is still open). We reach the raw SqliteConnection
			// and SqliteTransaction through the EF context using GetDbConnection() and
			// GetDbTransaction(); no new resources are opened.
			if (unifiedBookmarkPath && pendingBookmarks is not null)
			{
				SqliteConnection rawConn = (SqliteConnection)db.Database.GetDbConnection();
				SqliteTransaction rawTx = (SqliteTransaction)tx.GetDbTransaction();
				await _bookmarkStore!
					.SaveBatchInSameTransactionAsync(rawConn, rawTx, pendingBookmarks, ct)
					.ConfigureAwait(false);
			}

			await tx.CommitAsync(ct).ConfigureAwait(false);

			// Cache is already up-to-date (UpdateCache ran before BeginTransaction). Notify
			// the store that the commit landed — no-op today, reserved for observability.
			if (unifiedBookmarkPath && pendingBookmarks is not null)
			{
				foreach (string channel in pendingBookmarks.Keys)
				{
					_bookmarkStore!.MarkCommitted(channel);
				}
			}
			...
			{
				_logger.LogDebug(
					"EventProcessorWorker BATCH COMMIT: entities={EntityCount} addressesUpserted={Addresses} authFactsFailed={AuthFailed} authFactsSucceeded={AuthSucceeded} bookmarksCommitted={BookmarkCount}",
		...
		catch (Exception ex)
		{
			...
			// Restore the bookmark cache to its pre-batch value. We do this BEFORE re-throw
			// so any subsequent retry (WithRetryAsync in ExecuteAsync) sees the correct
			// baseline. If _bookmarkStore itself failed inside the try (e.g. SQLite disk
			// full), rollback still restores the pre-batch value — the store is a dumb-cache;
			// it doesn't distinguish 'failed here' from 'failed downstream'.
			if (unifiedBookmarkPath && previousBookmarks is not null)
			{
				foreach (KeyValuePair<string, string?> entry in previousBookmarks)
				{
					_bookmarkStore!.RollbackCache(entry.Key, entry.Value);
				}
			}

			if (debugEnabled)
			{
				_logger.LogDebug(ex, "EventProcessorWorker BATCH ROLLBACK: entities={EntityCount} bookmarksRolledBack={BookmarkCount}",
					entities.Count, previousBookmarks?.Count ?? 0);
			}
